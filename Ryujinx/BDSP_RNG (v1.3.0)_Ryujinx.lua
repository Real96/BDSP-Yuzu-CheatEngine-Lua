...
do
 ...
 seedAddressScan.firstScan(soExactValue, vtQword, 0, "04B7062830444F4D", "", 0, 0x7FFFFFFFFFFF,
                           "", fsmNotAligned, nil, true, false, false, false)
 seedAddressScan.waitTillDone()
 foundList.initialize()

 mainAddr = tonumber(foundList.Address[3], 16) - 0x8
 baseAddr = mainAddr - 0x8504000
...
local function getPlayerPrefsProviderInstanceAddr()
 local diamondPlayerPrefsProviderInstanceAddr = 0x4C90330
 local pearlPlayerPrefsProviderInstanceAddr = 0x4EA7408
...
local stateAddr = readQword(mainAddr + 0x4FD43D0) + baseAddr
...
local function getWildPokemonAddr()
 local wildPokemonAddr = readQword(playerPrefsProviderAddr + baseAddr + 0x800)
...
local function getPartyPokemonAddr()
 ...
 if viewMode[viewModeIndex] == "Breeding" then
  local partyMemberCountAddr = readQword(playerPrefsProviderAddr + baseAddr + 0x808)
  partySlotIndex = readBytes(partyMemberCountAddr + baseAddr + 0x18) - 1
 end

 local partyPokemonAddr = readQword(playerPrefsProviderAddr + baseAddr + 0x808)
